...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    data = pd.merge(orders, items, on='order_id', how="outer")
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    orders = pd.read_sql("SELECT * FROM olist_orders", database)
    ...
    order_purchase_ammount_per_date["order_purchase_timestamp"] =  pd.to_datetime(order_purchase_ammount_per_date["order_purchase_timestamp"])
